...
class QwenEmbeddingEncoder:
    ...
    def _validate_huggingface_token(self) -> bool:
        ...
        try:
            ...
            provider_info = f" (provider: {self.huggingface_provider})" if self.huggingface_provider else ""
            logger.info(f"正在验证 HuggingFace Token 并测试 Inference API{provider_info}...")
            ...
            if result is not None:
                ...
                logger.info(f"HuggingFace Inference API 验证成功，将使用远程服务")
                return True
            else:
                logger.warning(f"HuggingFace Inference API 返回空结果，将回退到本地模型")
                return False
            ...
            if "401" in error_msg or "unauthorized" in error_msg or "invalid" in error_msg:
                logger.warning(f"HuggingFace Token 无效或已过期，将回退到本地模型")
            elif "503" in error_msg or "loading" in error_msg:
                # 模型正在加载中，也视为可用
                logger.info(f"HuggingFace 模型正在加载中，将使用远程服务 (首次请求可能较慢)")
                try:
                    from huggingface_hub import InferenceClient
                    client_kwargs = {"api_key": self.huggingface_token}
                    if self.huggingface_provider:
                ...
                logger.warning(f"模型 {self.model_name} 不支持 HuggingFace Inference API，将回退到本地模型")
            else:
                logger.warning(
    ...
    def _initialize_local_model(self):
        ...
        if self._model is None:
            try:
                ...
                self._dimensions = len(test_embedding[0])
                logger.info(f"本地编码器初始化完成，设备: {self.device}，维度: {self._dimensions}")
            except Exception as e:
